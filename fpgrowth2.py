...
class Fp_growth():
    ...
    def create_fptree(self, data_set, min_support):  # 建树主函数
        '''
        根据data_set创建fp树
        header_table结构为
    ...
    def generate_L(self, data_set, min_support):
        freqItemSet = set()
        support_data = {}
        tree_header, headerTable = self.create_fptree(data_set, min_support)  # 创建数据集的fptree
...
if __name__ == "__main__":
    ...
    fp = Fp_growth()
    rule_list = fp.generate_R(datalist, min_support, min_conf)
    print("========================= 关联规则 =========================")
    index = 1
